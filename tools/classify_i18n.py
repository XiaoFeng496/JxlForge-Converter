...
# 自定义命令模板：示例文本现在**可以翻译**。
#
# 早期版本里 ``<输入>``/``<输出>`` 是硬编码字面量——校验用
# ``if "<输入>" not in raw``、替换用 ``.replace("<输入>", src)``，都不走 ``t()``，
# 所以示例翻成英文后用户照抄会过不了校验，当时整条列入 SKIP。
#
# 现已改为**双语兼容**：``_run_custom_command`` 与自定义命令校验同时接受
# 中文占位符 ``<输入>``/``<输出>`` 与英文占位符 ``<input>``/``<output>``
# （见 main_window.py 的解析与校验分支）。因此示例可以跟随界面语言显示，
# 英文用户照抄 ``cjxl <input> <output> -e 7 ...`` 也能正确校验通过。
# 故不再把示例排除出翻译集合，直接按普通界面文本处理（context 为 ``t`` / ``setText``）。
COMMAND_TEMPLATES = set()
